...
use anyhow::Result;
use askama::Template; // for #[derive(Template)]
use axum::{
    async_trait,
...
async fn validate_jwt(token: &str) -> Result<Claims> {
    // 1) Prefer server-side introspection (most robust, no local JWT assumptions)
    if let Ok(claims) = fetch_user_info_via_introspection(token).await {
        return Ok(claims);
    }
    // 2) Try UserInfo with bearer token (standard for access tokens)
    if let Ok(claims) = fetch_user_info_from_access_token(token).await {
        return Ok(claims);
    }
    // 3) Finally, try local JWT validation (for id_tokens)
...
fn try_validate_as_jwt(token: &str) -> Result<Claims> {
    ...
    // Configure validation with proper issuer check. Skip audience to support access tokens.
    let mut validation = Validation::new(Algorithm::RS256);
    validation.set_issuer(&[ISSUER]);
    validation.validate_aud = false; // don't enforce audience to avoid InvalidAudience loops
...
// Internal URLs (for service-to-service)
...
static USERINFO_URL: &str = "http://keycloak:8080/realms/todo-realm/protocol/openid-connect/userinfo";
static ISSUER: &str = "http://localhost:8080/realms/todo-realm";
static INTROSPECT_URL: &str = "http://keycloak:8080/realms/todo-realm/protocol/openid-connect/token/introspect";
...
// Global storage for PKCE verifiers (keyed by CSRF state)
...
async fn login() -> impl IntoResponse {
    ...
    // Store the PKCE verifier using CSRF token as key
    {
...
async fn callback(jar: CookieJar, Query(params): Query<AuthRequest>) -> impl IntoResponse {
    ...
    // Retrieve the PKCE verifier using the state parameter
    let state = match params.state.as_ref() {
        Some(state) => state.clone(),
        None => {
            // If we already have a session, just redirect home
    ...
    let pkce_verifier = {
        ...
        match verifiers.remove(&state) {
            ...
            None => {
                // If user already has a valid session cookie, ignore duplicate/late callbacks
                if jar.get("auth_token").is_some() {
                    return Redirect::to("/").into_response();
                }
    ...
    match &token_result {
        Ok(token) => {
            ...
            // Clean up the PKCE verifier after successful token exchange
            {
                let mut verifiers = PKCE_VERIFIERS.lock().await;
                verifiers.remove(&state);
            }

            // Set the auth cookie with secure flags and proper expiration
            let exp = chrono::Utc::now() + chrono::Duration::minutes(30);
            let cookie = format!(
                "auth_token={}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1800; Expires={}",
                token_for_cookie,
            ...
            // Use append to avoid overwriting when multiple Set-Cookie headers are present
            response
                .headers_mut()
            ...
            if let Some(refresh_token) = token.refresh_token() {
                let refresh_cookie = format!(
                    "refresh_token={}; Path=/; HttpOnly; SameSite=Lax; Max-Age=86400; Expires={}",
                    refresh_token.secret(),
                    (exp + chrono::Duration::days(1)).format("%a, %d %b %Y %H:%M:%S GMT")
                );
                // Append second cookie instead of replacing the first one
                response
                    .headers_mut()
...
async fn logout() -> impl IntoResponse {
    // Build RP-initiated logout URL at Keycloak, then redirect back to our /login
    // See: OpenID Connect RP-Initiated Logout (Keycloak end-session endpoint)
    let post_logout_redirect = "http%3A%2F%2Flocalhost%3A3000%2Flogin"; // urlencoded http://localhost:3000/login
    ...
    // Clear all auth-related cookies with proper flags on our domain
    let cookies = [
